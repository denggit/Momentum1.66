...
@Author     : Zijun Deng
@Date       : 3/7/26
@File       : smc_validator.py
@Description: SMC验证模块 - 独立的多时间级别结构分析器
              作为三号引擎的辅助识别模块，提供SMC结构验证和阻力位探测
"""
import asyncio
import datetime
...
class SMCValidator:
    ...
    def update_structure(self):
        ...
        try:
            ...
            logger.debug(f"🗺️ [SMC验证器] 多维结构更新！共 {len(self.active_pois)} 个复合POI，探明 {hvn_count} 座筹码峰。")

        except Exception as e:
            logger.exception(f"❌ [SMC验证器] 更新K线结构失败: {e}")
    ...
    def get_nearest_resistance(self, current_price: float):
        ...
        try:
            ...
            logger.debug(f"🎯 [SMC验证器] 锁定上方阻力 ({nearest['type']}): 目标价 {nearest['price']:.2f}")
            return nearest['price']

        except Exception as e:
            logger.error(f"❌ [SMC验证器] 寻找阻力位失败: {e}")

        return None

    async def background_update_loop(self):
        """🌟 后台静默守护进程：自动解析 timeframe，极其精确地对齐换线瞬间"""
        logger.info(f"📡 [SMC验证器] 已启动多维静默扫描！将精确对齐 5m 周期换线瞬间抓取全量 K 线...")

        # 解析当前的 timeframe 是多少分钟
        tf_minutes = 5

        # 🌟 核心修复：系统刚启动时，直接强制拉取一次，防止在等待下个 00 秒期间"瞎眼"
        logger.info("📡 [SMC验证器] 正在执行冷启动初次侦察...")
        ...
        while True:
            ...
            logger.debug(f"⏳ [SMC验证器] 正在休眠等待换线，距离下一次拉取还有 {sleep_seconds:.1f} 秒...")
...
# 向后兼容：保持 MicroSMCRadar 别名
MicroSMCRadar = SMCValidator
