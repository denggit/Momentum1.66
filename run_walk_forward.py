...
def run_walk_forward():
    print("🚀 启动机构级滚动回测引擎 (Walk-Forward Engine)...")
    ...
    print(f"📈 盲测范围: {OOS_START.date()} 至 {END_DATE.date()}")
    print(f"⚙️  滚动配置: 窗口 {TRAIN_WINDOW_YEARS}年 | 步进 {STEP_MONTHS}个月 | 阈值 {AI_THRESHOLD}")
    print("-" * 60)
    ...
    while current_test_start < END_DATE:
        ...
        if len(df_train) < 20:
            print(f"⚠️ {current_test_start.date()} 轮次样本太少 ({len(df_train)})，跳过...")
    ...
    print("\n" + "=" * 60)
    print("🏆 终极滚动回测报告 (2024-2025 样本外盲测)")
    print("=" * 60)
    print(f"📊 交易频次: 裸跑 {raw_trades} 次 -> AI 过滤后 {ai_trades} 次 (拦截了 {raw_trades - ai_trades} 笔垃圾单)")
    print(f"🎯 综合胜率: 裸跑 {raw_win_rate:.2f}% -> AI 过滤后 {ai_win_rate:.2f}%")
    print(f"💰 累计利润: 裸跑 ${raw_pnl:.2f} -> AI 过滤后 ${ai_pnl:.2f}")

    improvement = ((ai_pnl - raw_pnl) / abs(raw_pnl)) * 100 if raw_pnl != 0 else 0
    print(f"📈 AI 带来的利润增幅: {improvement:.2f}%")
    ...
    print(f"🛡️  2024 地狱年度表现: 裸跑 ${raw_2024:.2f} -> AI 拦截后 ${ai_2024:.2f}")
    print("=" * 60)
