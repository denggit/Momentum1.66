...
本项目旨在构建一个高可用、模块化的量化交易系统，专注于捕捉 **Ethereum (ETH)** 市场的“肥尾”效应（Fat-tail
trends）。系统核心基于 **Volatility Squeeze (波动率挤压)** 策略，结合严格的 **1.66% 固定风险定额**
资金管理模型，实现“极小回撤、爆发增长”的非对称收益曲线。
...
系统采用 **六边形架构 (Hexagonal Architecture)** 思想，核心逻辑与外部依赖（交易所 API、数据库）解耦，确保未来从 OKX 迁移至
Binance 时只需替换适配器。
